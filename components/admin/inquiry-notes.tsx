...
import { addInquiryNote } from '@/lib/actions/inquiries'
...
export function InquiryNotes({ inquiryId, initialNotes }: InquiryNotesProps) {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      const result = await addInquiryNote(inquiryId, noteContent)
        ...
        <div className="space-y-0">
          {notes.map((note, index) => (
            ...
            >
              ...
              <p className="mt-1 whitespace-pre-wrap text-sm text-warm-600 leading-relaxed">
                {note.content}
              </p>
            </div>
          ))}
